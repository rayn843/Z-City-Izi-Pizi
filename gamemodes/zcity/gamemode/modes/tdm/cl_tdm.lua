...
function MODE:HUDPaint()
	...
	if a_title > 0 then
		draw.SimpleText("ZBattle | " .. (self.PrintName or "Team Deathmatch"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * a_title), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
TDM_EndStatsPanel = nil

function RemoveEndTDMRoundStatsPanel()
    if IsValid(TDM_EndStatsPanel) then
        TDM_EndStatsPanel:Remove()
        TDM_EndStatsPanel = nil
    end
end
...
local function CreateEndRoundStatsPanel()
    ...
    TDM_EndStatsPanel = vgui.Create("DPanel")
    TDM_EndStatsPanel:SetPos(posX, posY)
    TDM_EndStatsPanel:SetSize(topW, topH + medalH + padding)
    TDM_EndStatsPanel:MakePopup()
    TDM_EndStatsPanel:SetKeyboardInputEnabled(false)
    TDM_EndStatsPanel:SetMouseInputEnabled(false)

    TDM_EndStatsPanel.Paint = function(self, pw, ph) end

    -- Панель 1: Верхняя горизонтальная плашка
    local pnl1 = vgui.Create("DPanel", TDM_EndStatsPanel)
    pnl1:SetPos(0, 0)
    ...
    -- Панель 2: Вертикальная карточка медали
    local pnl2 = vgui.Create("DPanel", TDM_EndStatsPanel)
    ...
    TDM_EndStatsPanel.statsLbl = statsLbl
    TDM_EndStatsPanel.pnl2 = pnl2
