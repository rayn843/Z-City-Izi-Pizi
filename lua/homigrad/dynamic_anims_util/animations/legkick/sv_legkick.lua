...
function PLAYER:LegAttack()
    -- Убрано условие not self:IsOnGround() чтобы разрешить начало атаки в воздухе
    if not self:Alive() or hg.GetCurrentCharacter(self):IsRagdoll() or self:GetNWFloat("InLegKick",0) > CurTime() or self:IsSprinting() then return end
    ...
    -- НОВОЕ: Модификатор урона от скорости (если игрок в воздухе)
    if not self:IsOnGround() then
        local plyVelocity = self:GetVelocity():Length()
        -- math.Clamp ограничивает множитель: 
        -- При скорости 0 (стоит на месте) удар будет слабее (0.2 от базы).
        -- При большой скорости (например, 300) урон будет увеличен (1.5 от базы).
        local forceMultiplier = math.Clamp(plyVelocity / 200, 0.2, 1.5)
        dmg = dmg * forceMultiplier
    end
    ...
    self:PlayCustomAnims(anim, true, speed, true, animstopAdjust, {
        [0.12] = function(self)
            ...
            -- Убрана отмена анимации при нахождении в воздухе
