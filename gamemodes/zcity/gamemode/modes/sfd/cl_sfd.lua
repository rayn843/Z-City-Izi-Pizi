...
function MODE:HUDPaint()
	...
	-- 1. Фон в зависимости от роли (fighter)
	local bgColor = Color(fighter.color1.r * 0.1, fighter.color1.g * 0.1, fighter.color1.b * 0.1, 255 * globalFade)
	...
	if a_title > 0 then
		draw.SimpleText("Superfighters 3D", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * a_title), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		playStageSound(0.5, "arccw_uc/common/10x25/fire-dist-10x25-pistol-ext-01.ogg")
	end

	-- 4. Роль
	local ColorRole = Color(fighter.color1.r, fighter.color1.g, fighter.color1.b, 255 * a_role)
	...
	local ColorObj = Color(fighter.color1.r, fighter.color1.g, fighter.color1.b, 255 * a_obj)
...
SFD_EndStatsPanel = nil

function RemoveEndSFDRoundStatsPanel()
    if IsValid(SFD_EndStatsPanel) then
        SFD_EndStatsPanel:Remove()
        SFD_EndStatsPanel = nil
    end
end
...
local function CreateEndRoundStatsPanel()
    ...
    SFD_EndStatsPanel = vgui.Create("DPanel")
    SFD_EndStatsPanel:SetPos(posX, posY)
    SFD_EndStatsPanel:SetSize(topW, topH + medalH + padding)
    SFD_EndStatsPanel:MakePopup()
    SFD_EndStatsPanel:SetKeyboardInputEnabled(false)
    SFD_EndStatsPanel:SetMouseInputEnabled(false)

    SFD_EndStatsPanel.Paint = function(self, pw, ph) end

    local pnl1 = vgui.Create("DPanel", SFD_EndStatsPanel)
    pnl1:SetPos(0, 0)
    ...
    local pnl2 = vgui.Create("DPanel", SFD_EndStatsPanel)
    ...
    SFD_EndStatsPanel.statsLbl = statsLbl
    SFD_EndStatsPanel.pnl2 = pnl2
