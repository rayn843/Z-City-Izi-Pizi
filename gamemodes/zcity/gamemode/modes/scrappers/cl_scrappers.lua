...
function MODE:HUDPaint()
    ...
    if extraction and extraction >= CurTime() then
        ...
        if extraction - CurTime() >= 60 and LastExtract then
            draw.SimpleText("Время ещё не подошло", "ZB_ScrappersMedium", sw * 0.01 + ScreenScale(0.75), sh * 0.97 + ScreenScale(0.75), color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText("Время ещё не подошло", "ZB_ScrappersMedium", sw * 0.01, sh * 0.97, ExtractionColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        end
