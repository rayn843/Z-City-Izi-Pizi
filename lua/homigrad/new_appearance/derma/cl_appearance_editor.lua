...
function PANEL:PostInit()
    ...
    function viewer:LayoutEntity( Entity )
        ...
        -- [FIX] Ensure necessary sub-tables exist (protects against outdated/corrupted presets)
        ...
        -- [FIX] Prevent crashes if the model saved in the preset was removed from the gamemode
        ...
        for k, v in SortedPairs(tMdl.submatSlots or {}) do
            local slot = 1
            for i = 1, #mats do
                if mats[i] == v then slot = i-1 break end
            end
            
            -- [FIX] Safely fetch clothes and handle potential nil strings for SetNWString
            local genderClothes = hg.Appearance.Clothes[tMdl.sex and 2 or 1]
            if genderClothes then
                local mat = genderClothes[tbl.AClothes[k]] or genderClothes["normal"]
                if mat then 
                    Entity:SetSubMaterial(slot, mat) 
                end
            end
            
            -- Fallback to an empty string to prevent a type mismatch error
            Entity:SetNWString("Colthes" .. k, tbl.AClothes[k] or "")
        end
