...
local function CreateRadialMenu(options_arg)
    -- Проверяем, не пытается ли игрок открыть спавн-меню (Q+R для админов)
    if isRPressed and isQPressed then
        -- Не создаем радиальное меню, если нажаты Q+R и игрок админ
        return
    end
...
hook.Add( "PlayerBindPress", "PlayerBindPressExample2huy", function( ply, bind, pressed )
    -- Обрабатываем Q для радиального меню
    if bind == "+menu" then -- Q по умолчанию
        if (lply.organism and lply.organism.otrub) then
            return true
        end

        -- Если нажат R одновременно - открываем спавн-меню (только для админов)
        if isRPressed then
            return false -- Позволяем Garry's Mod обработать спавн-меню
        end
        
        -- Иначе открываем радиальное меню
    ...
    -- Обрабатываем R
    if bind == "gm_showhelp" then -- R по умолчанию
        isRPressed = pressed
        -- Если нажат Q одновременно и игрок админ - открываем спавн-меню
        if isQPressed and pressed  then
            RunConsoleCommand("spawnmenu")
            return true
        end
        return false
    end
    
    -- Позволяем работать F1 (обычно спавн-меню)
    if bind == "gm_showspare1" or bind == "+menu_context" then
        return false
    end
end)

hook.Add("Think", "hg-radial-menu", function()
    -- Отслеживаем нажатие R
    isRPressed = input.IsKeyDown(KEY_T)
    
    -- Отслеживаем нажатие Q
    isQPressed = input.IsKeyDown(KEY_Q)
    ...
    -- Логика открытия меню по Q
    if input.IsKeyDown(KEY_Q) and not isRPressed then
        if firstTime then
            firstTime = false
        end
        firstTime4 = true
    ...
    -- Логика для Q+R (спавн-меню для админов)
    if input.IsKeyDown(KEY_Q) and input.IsKeyDown(KEY_R) then
        if firstTime then
            firstTime = false
            -- Открываем спавн-меню
            RunConsoleCommand("spawnmenu")
        end
        firstTime4 = true
    end
    
    -- Остальная логика для мыши (не изменяется)
