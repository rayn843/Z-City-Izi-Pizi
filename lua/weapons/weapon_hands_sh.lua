...
function SWEP:SetHandPos(noset)
    ...
    local hasShield = ply:GetNWBool("HasShield", false) and not IsValid(ply:GetNetVar("carryent"))

    -- Блокируем стандартные анимации, если держим щит
    if not hasShield and ply:GetNetVar("handcuffed",false) then
    ...
    -- ПОЛНЫЙ IK КОНТРОЛЬ ДЛЯ ЩИТА
    if hasShield then
        local chestBone = ply:LookupBone("ValveBiped.Bip01_Spine4")
        if chestBone then
            local chestPos = ply:GetBoneMatrix(chestBone):GetTranslation()
            local eyeang = ply:EyeAngles()
            
            -- НАСТРОЙКИ ПОЗИЦИИ И УГЛА
            local posShieldL = Vector(17, 2, -3)
            local posShieldR = Vector(14, -4, 0)
            local angShieldL = Angle(30, -75, 10)
            local angShieldR = Angle(-90, 0, 90)

			local bashTime = self.shieldBashTime or 0
            if bashTime > CurTime() then
                local progress = 1 - math.Clamp((bashTime - CurTime()) / 0.15, 0, 1)
                -- Выталкиваем левую руку вперед (по оси X) на 30 юнитов
                local pushForward = math.ease.InQuad(progress) * 6
                posShieldL.x = posShieldL.x + pushForward
				posShieldR.x = posShieldR.x + pushForward
            end
            ...
            if lh then
                ...
                if lhmat then
                    local targetPosL = chestPos + eyeang:Forward() * posShieldL.x + eyeang:Right() * posShieldL.y + eyeang:Up() * posShieldL.z
                    lhmat:SetTranslation(targetPosL)
                    
                    -- Берем стабильный угол от взгляда игрока (чтобы не было анимации бега)
                    local handAngL = Angle(eyeang.p, eyeang.y, eyeang.r)
                    handAngL:RotateAroundAxis(handAngL:Forward(), angShieldL.r)
                    handAngL:RotateAroundAxis(handAngL:Right(), angShieldL.p)
                    handAngL:RotateAroundAxis(handAngL:Up(), angShieldL.y)
                    lhmat:SetAngles(handAngL)
                    
                    if hg.bone_apply_matrix then
                        hg.bone_apply_matrix(ply, lh, lhmat)
                    else
                        ply:SetBoneMatrix(lh, lhmat)
                    end
            ...
            -- ПРАВАЯ РУКА (За щитом)
            local rh = ply:LookupBone("ValveBiped.Bip01_R_Hand")
            if rh then
                local rhmat = ply:GetBoneMatrix(rh)
                if rhmat then
                    local targetPosR = chestPos + eyeang:Forward() * posShieldR.x + eyeang:Right() * posShieldR.y + eyeang:Up() * posShieldR.z
                    rhmat:SetTranslation(targetPosR)
                    
                    local handAngR = Angle(eyeang.p, eyeang.y, eyeang.r)
                    handAngR:RotateAroundAxis(handAngR:Forward(), angShieldR.r)
                    handAngR:RotateAroundAxis(handAngR:Right(), angShieldR.p)
                    handAngR:RotateAroundAxis(handAngR:Up(), angShieldR.y)
                    rhmat:SetAngles(handAngR)
                    
                    if hg.bone_apply_matrix then
                        hg.bone_apply_matrix(ply, rh, rhmat)
                    else
                        ply:SetBoneMatrix(rh, rhmat)
                    end
                end
            end
        end
        -- Мы НЕ делаем return, чтобы hg.DragHands ниже притянул предплечья к нашим кистям
    elseif self:GetFists() then
        ...
        local targetR = (self:GetBlocking() and vecBlockingR or vecR)
        self.blockingR = LerpFT(0.1, self.blockingR or vecR, targetR)
        ...
        if self.rhandik then
            for _, bone in ipairs(bones) do
                ...
                bonepos.x = clamp(bonepos.x, wmpos.x - 38, wmpos.x + 38)
        ...
        local targetL = (self:GetBlocking() and vecBlockingL or vecL)
        self.blockingL = LerpFT(0.1, self.blockingL or vecL, targetL)
    ...
    -- Отключаем кастет, если держим щит
    if IsValid(ply) and havekastet and not hasShield and not IsValid(ply:GetNetVar("carryent")) then
        ...
        kastet:SetupBones()
        kastet:DrawModel()
        kastet:SetModelScale(0.9)
    end

    -- Притягиваем предплечья к нашим кистям (очень важно для щита)
    ...
    if trace2.Hit and not (trace2.Entity:IsPlayer() or trace2.Entity:IsNPC()) then end
...
function SWEP:Think()
    ...
    -- Автоматически поднимаем кулаки, если в руках щит (для активации хитбокса)
    if owner:GetNWBool("HasShield", false) and not self:GetFists() and not IsValid(owner:GetNetVar("carryent")) then
        self:SetFists(true)
        self:DoBFSAnimation("fists_draw",1)
    end

    -- Если держим щит, запрещаем блокировать кулаками (чтобы не было конфликтов анимаций)
    if owner:GetNWBool("HasShield", false) then
        self:SetBlocking(false)
    end
    ...
    if self:GetFists() then
        ...
        if owner:KeyDown(IN_SPEED) and (owner.PlayerClassName != "furry" or owner:KeyDown(IN_WALK)) and owner.PlayerClassName ~= "headcrabzombie" then
            -- Запрещаем опускать руки при спринте, если держим щит
            if not owner:GetNWBool("HasShield", false) then
                self:SetNextDown(Time + 1)
                self:SetFists(false)
                self:SetBlocking(false)
            end
...
function SWEP:ShieldBash()
    local owner = self:GetOwner()
    if not IsValid(owner) then return end
    if (self.shieldBashCd or 0) > CurTime() then return end

    -- Кулдаун и таймер для анимации выталкивания
    self.shieldBashCd = CurTime() + 2
    self.shieldBashTime = CurTime() + 0.5
    
    -- Задержка перед следующим действием
    self:SetNextPrimaryFire(CurTime() + 1)

    -- Отдача камеры для локального игрока
    ...
    if SERVER then
		timer.Simple(0.35, function()
            owner:EmitSound("pwb2/weapons/famasg2/cloth.wav", 65, math.random(80, 120))
        end)

        -- Трейс для удара (длина 70 юнитов)
		
        local pos = hg.eye(owner)
        local tr = util.TraceHull({
            start = pos,
            endpos = pos + owner:GetAimVector() * 70,
        ...
        if tr.Hit and IsValid(tr.Entity) then
			timer.Simple(0.35, function()
				...
				dmg:SetDamage(10) -- Урон от удара щитом
				...
				-- Отбрасывание и звук удара
				if ent:IsPlayer() or ent:IsNPC() then
					ent:SetVelocity(owner:GetAimVector() * 250)
					ent:EmitSound("physics/body/body_medium_impact_hard" .. math.random(1, 6) .. ".wav", 70)
				else
					local phys = ent:GetPhysicsObject()
					if IsValid(phys) then
						phys:ApplyForceOffset(owner:GetAimVector() * 1000, tr.HitPos)
					end
				end
			end)
        end
