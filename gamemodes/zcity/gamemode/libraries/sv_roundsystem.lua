...
local hasChangeLevel = false
local checkedChangeLevel = false

zb.ROUND_STATE = zb.ROUND_STATE or 0
zb.CROUND = zb.CROUND or "hmcd"
zb.nextround = zb.nextround or "hmcd"
...
function zb:GetMode(round)
    ...
    -- Приоритет для основного режима Homicide
    if zb.modes["hmcd"] and zb.modes["hmcd"].Types and zb.modes["hmcd"].Types[round] then
        return "hmcd"
    end

    for name, mode in pairs(zb.modes) do
        if name ~= "fear" and mode.Types and mode.Types[round] then
            return name
        end
    end

    -- Если режим не найден, возвращаем запасной режим, чтобы избежать крашей
    return "hmcd" 
end

function CurrentRound()
    if not checkedChangeLevel then
        hasChangeLevel = IsValid(ents.FindByClass("trigger_changelevel")[1])
        checkedChangeLevel = true
    end

    --- ДОБАВЛЕНО: Игнорируем trigger_changelevel на карте gm_suruba_67
    if hasChangeLevel and game.GetMap() ~= "gm_suruba_67" then
        zb.nextround = "coop"
    ...
    if not zb.CROUND_MAIN or (zb.LASTCROUND != zb.CROUND) then
        ...
        -- ЗАЩИТА: Если режима не существует, откатываемся на hmcd
        if not zb.modes[zb.CROUND_MAIN] then
            zb.CROUND_MAIN = "hmcd"
            if not zb.modes[zb.CROUND_MAIN] then
                local fallback = next(zb.modes)
                if fallback then
                    zb.CROUND_MAIN = fallback
                    zb.CROUND = fallback
                end
            end
        end
...
function NextRound(round)
    --- ДОБАВЛЕНО: Игнорируем trigger_changelevel на карте gm_suruba_67
    if IsValid(ents.FindByClass( "trigger_changelevel" )[1]) and game.GetMap() ~= "gm_suruba_67" then
        zb.nextround = "coop"
    else
        if round and (zb.modes[round] or zb:GetMode(round)) then
            zb.nextround = round
        else
            print("[ZB] Предупреждение: Попытка установить несуществующий раунд '" .. tostring(round) .. "'. Возврат к 'hmcd'.")
            zb.nextround = "hmcd"
        end
    end
end
...
function zb:PreRound()
    --- ДОБАВЛЕНО: Проверка лимитов для карты gm_suruba_67
    if game.GetMap() == "gm_suruba_67" then
        local timeExpired = (CurTime() - (zb.SurubaStartTime or CurTime())) >= 3600
        local smoExhausted = (zb.SurubaModePlays.smo or 0) >= 3
        -- scrappersExhausted удален
        local modesExhausted = smoExhausted

        if timeExpired or modesExhausted then
            if zb.ROUND_STATE == 0 and #player_GetAll() > 1 then
                if not zb.SurubaRTVStarted then
                    local reason = timeExpired and "время карты (60 мин) истекло" or "лимиты раундов исчерпаны"
                    PrintMessage(HUD_PRINTTALK, "[ZB] " .. reason .. ". Запускаем голосование за смену карты...")
                    zb.StartRTV(20)
                    zb.SurubaRTVStarted = true
                end
            end
            return -- Блокируем запуск новых раундов
        end
    end
...
hook.Add("InitPostEntity", "loadbigmap", function()
    ...
    --- ДОБАВЛЕНО: Инициализация переменных при загрузке карты gm_suruba_67
    if game.GetMap() == "gm_suruba_67" then
        zb.SurubaModePlays = { smo = 0 }
        zb.SurubaStartTime = CurTime()
        zb.SurubaRTVStarted = false
        
        forcemodeconvar:SetString("random")
        forcemode = "random"
        
        print("[ZB] Карта gm_suruba_67 загружена. Ограничения: smo (3 раунда), лимит времени 60 минут.")
    end
end)
...
function zb.GetAvailableModes()
    ...
    for i, name in pairs(zb.GetModes()) do
        ...
        -- Жесткая фильтрация дубликата homicide2
        if name == "fear" then continue end
        
        --- ДОБАВЛЕНО: Жесткая фильтрация режимов для gm_suruba_67 (Игнорируем CanLaunch)
        if game.GetMap() == "gm_suruba_67" then
            if tbl.SubModes then
                for i, name2 in pairs(tbl:SubModes()) do
                    if name2 == "smo" then
                        if name2 == "smo" and (zb.SurubaModePlays.smo or 0) >= 3 then continue end
                        table.insert(newtbl, name2)
                    end
                end
            else
                if name == "smo" then
                    if name == "smo" and (zb.SurubaModePlays.smo or 0) >= 3 then continue end
                    table.insert(newtbl, name)
                end
            end
            continue
...
function zb.AddCurrentModePlayed()
    ...
    --- ДОБАВЛЕНО: Увеличение счетчиков раундов для gm_suruba_67
    if game.GetMap() == "gm_suruba_67" then
        if name == "smo" then
            zb.SurubaModePlays.smo = (zb.SurubaModePlays.smo or 0) + 1
            PrintMessage(HUD_PRINTTALK, "[ZB] Режим smo сыгран " .. zb.SurubaModePlays.smo .. "/3 раз.")
        end
    end
end
...
--- ИЗМЕНЕНО: Жесткое формирование списка раундов для gm_suruba_67
function zb.RerollChances()
    ...
    if game.GetMap() == "gm_suruba_67" then
        ...
        local smo_left = math.max(0, 3 - (zb.SurubaModePlays.smo or 0))
        
        for i = 1, smo_left do table.insert(surubaList, "smo") end
        
        table.Shuffle(surubaList)
        
        zb.RoundList = surubaList
        zb.nextround = #zb.RoundList > 0 and table.remove(zb.RoundList, 1) or "hmcd"
        return
    end
