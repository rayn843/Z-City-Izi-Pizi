...
function SWEP:Camera(eyePos, eyeAng, view, vellen, ply)
    ...
    local animpos = (self.AdditionalAng or Angle(0, 0, 0))[2] / 20 + (self.AdditionalAng2 or Angle(0, 0, 0))[2] / 20
	
	-- Узнаем, разложены ли сошки. Если да, ставим множитель 0.15 (15% отдачи)
	local isResting = self:GetNWBool("IsResting", false)
	local bipodMul = isResting and 0.15 or 1 

	-- Домножаем базовый увод камеры на наш множитель
	local eyeSpray = -(-self.EyeSpray) * bipodMul
    ...
    angle_spray[3] = math.Rand(-self.sprayAngles[3], self.sprayAngles[3]) * 60 * game.GetTimeScale() * 0.7 * bipodMul
	angle_spray[1] = math.Rand(-self.sprayAngles[3], self.sprayAngles[3]) * 12 * game.GetTimeScale() * 0.7 * bipodMul
	angle_spray[2] = math.Rand(-self.sprayAngles[3], self.sprayAngles[3]) * 12 * game.GetTimeScale() * 0.7 * bipodMul
