...
function hg.organism.AddWound(ent, tr, bone, dmgInfo, dmgPos, dmgBlood, inputHole, outputHole)
    ...
    if bone and dmgBlood > 0 then
        for i = 1, 2 do
            ...
            -- СОРТИРОВКА ОТСЮДА УДАЛЕНА для экономии тиков процессора
            ...
            if #org.wounds <= 30 then
                ...
                timer.Create("WoundsSend"..ent:EntIndex(), 0.1, 1, function()
                    local ent = org.owner
                    if IsValid(ent) then
                        -- Сортируем ОДИН раз прямо перед отправкой клиентам
...
function hg.organism.AddWoundManual(ent, dmgBlood, localPos, localAng, bone, time)
    ...
    if #org.wounds <= 30 then
        ...
        timer.Create("WoundsSend"..ent:EntIndex(), 0.1, 1, function()
            local ent = org.owner
            if IsValid(ent) then
                -- Сортируем ОДИН раз перед записью в NetVar
                table.sort(wounds, function(a, b) return a[1] > b[1] end)
                ent:SetNetVar("wounds", wounds)
                if IsValid(ent.RagdollDeath) then ent.RagdollDeath:SetNetVar("wounds", wounds) end
            end
...
hook.Add("EntityTakeDamage", "homigrad-damage", function(ent, dmgInfo)
	...
	if !dmgInfo:IsDamageType(DMG_BURN) then 
		local entID = org.owner:EntIndex()
		org.owner.fullsend = true
		
		-- Проверяем лимит времени, чтобы не спамить net-сообщениями
		org.next_net_sync = org.next_net_sync or 0
		if CurTime() >= org.next_net_sync then
			-- Если время пришло, отправляем пакет сразу и ставим задержку в 0.1 сек
			org.next_net_sync = CurTime() + 0.1
			hg.send_bareinfo(org) 
			if IsValid(org.owner) and org.owner.Alive and org.owner:Alive() then 
				hg.send_organism(org, org.owner) 
			end
		else
			-- Если урон наносится слишком часто, обновляем существующий таймер.
			-- Таким образом отправится только ОДИН финальный пакет по окончании серии попаданий.
			timer.Create("send_info_org"..entID, 0.1, 1, function() 
				if !IsValid(org.owner) then return end 
				hg.send_bareinfo(org) 
				if IsValid(org.owner) and org.owner.Alive and org.owner:Alive() then 
					hg.send_organism(org, org.owner) 
