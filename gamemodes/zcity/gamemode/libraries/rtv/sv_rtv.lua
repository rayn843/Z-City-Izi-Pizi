...
function zb.EndRTV()
    ...
    endStarted = true -- Сразу ставим флаг, чтобы не вызвать дважды

    local winmap = table.GetWinningKey(votes)
    
    -- Если никто не проголосовал, выбираем случайную карту
    if not winmap or winmap == "" then
        if #mappull > 0 then
            winmap = mappull[math.random(#mappull)]
        else
            winmap = "gm_construct"
        end
        print("[RTV] Никто не проголосовал. Выбрана случайная карта: " .. winmap)
    end
...
function zb.StartRTV(time)
    ...
    -- === НОВЫЙ КОД: ПРЕДОХРАНИТЕЛЬ ===
    -- Если неигранных карт осталось меньше 12 (столько система пытается засунуть в меню),
    -- или карт на сервере вообще меньше 12, сбрасываем историю PlayedMaps.
    if (#mappull - #PlayedMaps) < 1 then
        PlayedMaps = {}
        file.Write(playedMapsPath, util.TableToJSON(PlayedMaps))
    end
    -- ===================================
