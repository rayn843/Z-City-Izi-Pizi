...
net.Receive("hl2dm_start",function()
    ...
    RemoveHL2DM_EndRoundStatsPanel()
...
function MODE:HUDPaint()
    ...
    local bgColor = Color(team_data.color1.r * 0.1, team_data.color1.g * 0.1, team_data.color1.b * 0.1, 255 * globalFade)
    ...
    if a_title > 0 then
        draw.SimpleText("ZBattle | Half-Life 2 Deathmatch", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * a_title), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        playStageSound(0.5, "arccw_uc/common/10x25/fire-dist-10x25-pistol-ext-01.ogg")
    end

    -- 4. Роль (имя и цвет берутся из данных команды)
    local role_data = {
        name = team_data.name,
        color = Color(team_data.color1.r, team_data.color1.g, team_data.color1.b, 255 * a_role),
    ...
    local objective_color = Color(team_data.color2.r, team_data.color2.g, team_data.color2.b, 255 * a_obj)
...
HL2DM_EndStatsPanel = nil

function RemoveHL2DM_EndRoundStatsPanel()
    if IsValid(HL2DM_EndStatsPanel) then
        HL2DM_EndStatsPanel:Remove()
        HL2DM_EndStatsPanel = nil
    end
end
...
local function CreateEndRoundStatsPanel()
    ...
    HL2DM_EndStatsPanel = vgui.Create("DPanel")
    HL2DM_EndStatsPanel:SetPos(posX, posY)
    HL2DM_EndStatsPanel:SetSize(topW, topH + medalH + padding)
    HL2DM_EndStatsPanel:MakePopup()
    HL2DM_EndStatsPanel:SetKeyboardInputEnabled(false)
    HL2DM_EndStatsPanel:SetMouseInputEnabled(false)

    HL2DM_EndStatsPanel.Paint = function(self, pw, ph) end

    local pnl1 = vgui.Create("DPanel", HL2DM_EndStatsPanel)
    pnl1:SetPos(0, 0)
    ...
    local pnl2 = vgui.Create("DPanel", HL2DM_EndStatsPanel)
    ...
    HL2DM_EndStatsPanel.statsLbl = statsLbl
    HL2DM_EndStatsPanel.pnl2 = pnl2
