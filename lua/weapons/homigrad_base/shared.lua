...
SWEP.BipodBones = {
	{ bone = "bipod1", ang = Angle(35, 0, 0) },
	{ bone = "bipod2", ang = Angle(-35, 0, 0) }
}
...
function SWEP:Think()
    ...
    if CLIENT then
		...
		-- Проверяем, что ворлд-модель существует и это сущность (а не boolean)
		if IsValid(wm) and type(wm) ~= "boolean" then
			
			-- Проверяем наличие таблицы с настройками костей
			if self.BipodBones then
				
				-- Берем статус установки на сошки (из твоей базы)
				local isResting = self:GetNWBool("IsResting", false) 
				
				-- Плавная интерполяция от 0 до 1
				self.bipodLerp = Lerp(FrameTime() * 8, self.bipodLerp or 0, isResting and 1 or 0)

				-- Проходимся по всем костям
				for _, bipodInfo in ipairs(self.BipodBones) do
					local boneID = wm:LookupBone(bipodInfo.bone)
					
					if boneID then
						-- Высчитываем текущий угол поворота
						local currentAng = LerpAngle(self.bipodLerp, Angle(0,0,0), bipodInfo.ang)
						-- Применяем угол к кости ворлд-модели
						wm:ManipulateBoneAngles(boneID, currentAng)
					end
				end
			end
		end
...
hg.postureFunctions2 = {
	...
	[3] = function(self,ply,force)
		...
		self.AdditionalPosPreLerp[2] = self.AdditionalPosPreLerp[2] + (2 + (pistolRun and (isLocal and (epicRunZ or (running and 6 or 2)) - 6 or 4) or (isLocal and -2 or -6 + (ply:GetNWFloat("InLegKick", 0) and 5 or 0)) )) * runmul
		self.AdditionalPosPreLerp[1] = self.AdditionalPosPreLerp[1] - (-4 + (pistolRun and (isLocal and (epicRunY or (running and 6 or 4)) + 3 or 8 + (ply:GetNWFloat("InLegKick", 0) and -5 or 0)) or (isLocal and 8 or 2 + (ply:GetNW2Float("InLegKick", 0) and 8 or 0)) ) + 3 * math.Clamp(-ply:EyeAngles()[1] / 20, self:IsPistolHoldType() and -1 or -1, 0)) * runmul
...
function SWEP:GetAdditionalValues()
	...
	-- Целевые значения поворота/смещения (0 = нет столкновения, плавный возврат)
	local targetPitch, targetYaw, targetPush, targetRoll = 0, 0, 0, 0

	if not ply:InVehicle() and not huya and not self:IsSprinting() and self.closeanimtr then
		local tr = self.closeanimtr
		local hitNormal = tr.HitNormal  
		local aimVec = ply:GetAimVector()
		local wepAng = aimVec:Angle()
		local dot = aimVec:Dot(hitNormal)
		local perp = hitNormal - aimVec * dot
		local perpRight = perp:Dot(wepAng:Right())
		local perpUp = perp:Dot(wepAng:Up())
		local lengthScale = math.Clamp(self.closeanimdis / 100, 0.6, 1.4)
		local MAX_ANGLE = 55 * lengthScale
		local vel = ply:GetVelocity()
		local velTowardWall = math.max(-vel:Dot(hitNormal), 0)
		local velScale = 2.5 + math.Clamp(velTowardWall / 600, 0, 0.5)
		targetYaw = -MAX_ANGLE * perpRight * animpos * velScale
		targetPitch = -MAX_ANGLE * perpUp * animpos * velScale
		targetPush = math.max(-dot, 0) * animpos * self.closeanimdis
		targetRoll = -animpos * 4 * velScale
	end
	local dt = math.Clamp(dtime, 0, 1 / 10)

	local function springDamper(pos, vel, target, k, dampingRatio)
		local omega = math.sqrt(k)
		local c = dampingRatio * 2 * omega
		local steps = math.max(1, math.ceil(omega * dt / 0.1))
		local sdt = dt / steps
		for _ = 1, steps do
			local force = -k * (pos - target) - c * vel
			vel = vel + force * sdt
			pos = pos + vel * sdt
		end
		return pos, vel
	...
	local function updateAxis(posKey, velKey, target)
		local pos = self[posKey] or 0
		local vel = self[velKey] or 0
		local k, ratio
		if math.abs(target) > math.abs(pos) + 0.5 then
			k, ratio = 300, 1.0   -- impact: быстро, без переброса
		else
			k, ratio = 150, 0.6   -- release: медленнее, лёгкий пружинный переброс
		end
		pos, vel = springDamper(pos, vel, target, k, ratio)
		self[posKey] = pos
		self[velKey] = vel
		return pos
	end

	local pitchOut = updateAxis("wallColPitch", "wallColPitchVel", targetPitch)
	local yawOut = updateAxis("wallColYaw", "wallColYawVel", targetYaw)
	local pushOut = updateAxis("wallColPush", "wallColPushVel", targetPush)
	local rollOut = updateAxis("wallColRoll", "wallColRollVel", targetRoll)

	-- Применяем. Позиция модели корректируется автоматически через
	-- LocalToWorld(self.WorldPos, ...) в WorldModel_Transform — оружие вращается
	-- вокруг рукояти (desiredPos), а не вокруг модельного origin.
	self.CloseAnimAddAng[1] = self.CloseAnimAddAng[1] + pitchOut
	self.CloseAnimAddAng[2] = self.CloseAnimAddAng[2] + yawOut
	self.CloseAnimAddAng[3] = self.CloseAnimAddAng[3] + rollOut
	self.CloseAnimAddVec[1] = self.CloseAnimAddVec[1] - pushOut
