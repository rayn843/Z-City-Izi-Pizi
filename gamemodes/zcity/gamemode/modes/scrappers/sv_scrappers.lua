...
-- Заменяем в sv_scrappers.lua обработчик zb_SellItem
net.Receive("zb_SellItem", function(len, ply)
    local slot = net.ReadString()   -- категория в инвентаре ("Primary", "Other"...)
    local pos = net.ReadUInt(16)    -- позиция в таблице
    local Inventory = ply:GetLocalVar("zb_Scrappers_Inventory", {})
    if not Inventory[slot] or not Inventory[slot][pos] then return end
    local itemClass = Inventory[slot][pos]
    
    -- Ищем цену во всех категориях магазина
    local price = nil
    for cat, items in pairs(MODE.ShopList) do
    ...
    local sellPrice = price - (price * 0.5)  -- 50% от цены
    ply:SetLocalVar("zb_Scrappers_Money", ply:GetLocalVar("zb_Scrappers_Money", MODE.StartingMoney) + sellPrice)
    table.remove(Inventory[slot], pos)
    ply:SetLocalVar("zb_Scrappers_Inventory", Inventory)
end)

zb.slotFunctions = {
    ["Primary"] = function(RaidInventory, Inventory, originalSlot, pos, targetSlot)
        if RaidInventory[targetSlot] ~= nil then return end
        RaidInventory[targetSlot] = Inventory[originalSlot][pos]
        Inventory[originalSlot][pos] = nil
    end,
    ["Secondary"] = function(RaidInventory, Inventory, originalSlot, pos, targetSlot)
        if RaidInventory[targetSlot] ~= nil then return end
        RaidInventory[targetSlot] = Inventory[originalSlot][pos]
        Inventory[originalSlot][pos] = nil
    end,
    ["Melee"] = function(RaidInventory, Inventory, originalSlot, pos, targetSlot)
        RaidInventory[targetSlot] = RaidInventory[targetSlot] or {}
        if table.HasValue(RaidInventory[targetSlot], Inventory[originalSlot][pos]) then return end
        RaidInventory[targetSlot][#RaidInventory[targetSlot] + 1] = Inventory[originalSlot][pos]
        Inventory[originalSlot][pos] = nil
    end,
    ["Medicine"] = function(RaidInventory, Inventory, originalSlot, pos, targetSlot)
        RaidInventory[targetSlot] = RaidInventory[targetSlot] or {}
        if table.HasValue(RaidInventory[targetSlot], Inventory[originalSlot][pos]) then return end
        RaidInventory[targetSlot][#RaidInventory[targetSlot] + 1] = Inventory[originalSlot][pos]
        Inventory[originalSlot][pos] = nil
    end,
    ["Other"] = function(RaidInventory, Inventory, originalSlot, pos, targetSlot)
        RaidInventory[targetSlot] = RaidInventory[targetSlot] or {}
        if table.HasValue(RaidInventory[targetSlot], Inventory[originalSlot][pos]) then return end
        RaidInventory[targetSlot][#RaidInventory[targetSlot] + 1] = Inventory[originalSlot][pos]
        Inventory[originalSlot][pos] = nil
    end,
    ["Armor"] = function(RaidInventory, Inventory, originalSlot, pos, targetSlot)
        RaidInventory[targetSlot] = RaidInventory[targetSlot] or {}
        local armorPlace = hg.GetArmorPlacement(Inventory[originalSlot][pos])
        if RaidInventory[targetSlot][armorPlace] then return end
        RaidInventory[targetSlot][armorPlace] = Inventory[originalSlot][pos]
        Inventory[originalSlot][pos] = nil
    end,
}

net.Receive("zb_FromInvToRaid", function(len, ply)
    local originalSlot = net.ReadString()
    local pos = net.ReadUInt(16)
    ...
    if not Inventory[originalSlot] or not Inventory[originalSlot][pos] then return end

    local item = hg.GetItem(Inventory[originalSlot][pos])
    local targetSlot = item.ScrappersSlot or "Other"

    -- ВНИМАНИЕ: Строка RaidInventory[targetSlot] = RaidInventory[targetSlot] or {} была удалена отсюда!

    if zb.slotFunctions[targetSlot] then
        zb.slotFunctions[targetSlot](RaidInventory, Inventory, originalSlot, pos, targetSlot)
    else
        RaidInventory[targetSlot] = RaidInventory[targetSlot] or {}
        table.insert(RaidInventory[targetSlot], Inventory[originalSlot][pos])
        Inventory[originalSlot][pos] = nil
    end
...
net.Receive("zb_FromRaidToInv", function(len, ply)
    local raidSlot = net.ReadString()   -- "Primary", "Secondary" и т.д.
    ...
    if not RaidInventory[raidSlot] then return end

    local itemClass
    local isArray = istable(RaidInventory[raidSlot]) and #RaidInventory[raidSlot] > 0

    if isArray then
        if not RaidInventory[raidSlot][pos] then return end
        itemClass = RaidInventory[raidSlot][pos]
    else
        -- одиночный предмет (Primary или Secondary)
        itemClass = RaidInventory[raidSlot]
        pos = nil
    end

    local item = hg.GetItem(itemClass)
    local invSlot = item.ScrappersSlot or "Other"

    Inventory[invSlot] = Inventory[invSlot] or {}
    table.insert(Inventory[invSlot], itemClass)

    -- удаляем из рейдового инвентаря
    if isArray then
        table.remove(RaidInventory[raidSlot], pos)
        if #RaidInventory[raidSlot] == 0 then
            RaidInventory[raidSlot] = nil
        end
    else
        RaidInventory[raidSlot] = nil
    end
